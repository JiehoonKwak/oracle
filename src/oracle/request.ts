...
export function buildRequestBody({
  ...
  const tools: ToolConfig[] = modelConfig.searchTools
    ? modelConfig.searchTools.map((t) => ({ type: t }))
    : [{ type: modelConfig.searchToolType ?? "web_search_preview" }];
  return {
    ...
    tools: searchEnabled ? tools : undefined,
