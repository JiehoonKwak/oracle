...
describe("oracle CLI integration", () => {
  ...
  test(
    "passes through gpt-5.1-codex-max as an unknown model",
    async () => {
      ...
      const metadataPath = path.join(sessionsDir, sessionIds[0], "meta.json");
      const metadata = JSON.parse(await readFile(metadataPath, "utf8"));
      expect(metadata.model).toBe("gpt-5.1-codex-max");
  ...
  test(
    "passes through exact model names in multi-model list",
    async () => {
      const oracleHome = await mkdtemp(path.join(os.tmpdir(), "oracle-multi-exact-"));
      const env = {
        ...process.env,
        // biome-ignore lint/style/useNamingConvention: env var name
        OPENAI_API_KEY: "sk-integration",
      ...
      await execFileAsync(
        ...
        [
          ...
          CLI_ENTRY,
          "--prompt",
          "Exact model names in multi-model prompt that is safely over twenty characters.",
          "--models",
          "gpt-5.1,gemini-3-pro,claude-4.5-sonnet",
        ],
        { env },
