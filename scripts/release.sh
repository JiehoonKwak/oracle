...
phase_gates() {
  ...
  run "$RUNNER" npm run check
  run "$RUNNER" npm run lint
  run "$RUNNER" npm run test
  run "$RUNNER" npm run build
}
...
phase_publish() {
  ...
  run "$RUNNER" npm publish --tag latest --access public
...
Phases (run individually or all):
  gates      npm check, lint, test, build
  artifacts  npm pack + sha1/sha256
  publish    npm publish --tag latest --access public, verify npm view
