...
export async function runOracle(options, deps = {}) {
    ...
    // ── Per-model routing ──
    function resolveRouting(model, explicitBaseUrl, env = process.env) {
        if (explicitBaseUrl) {
    ...
    if (!apiKey) {
        throw new PromptValidationError(`Missing ${envVar}. Set it via the environment or a .env file.`, { env: envVar });
    }
    ...
    const clientInstance = client ??
        clientFactory(apiKey, {
            baseUrl: apiEndpoint,
            model: options.model,
            resolvedModelId: modelProviderForDispatch === "google"
