...
export function toTransportError(error, model) {
    ...
    if (isApiError) {
        ...
        if (model === "gpt-5.2-pro" &&
            ...
            return new OracleTransportError("model-unavailable", "gpt-5.2-pro is not available on this API base/key. Try gpt-5-pro or gpt-5.2.", apiError);
        }
        if (apiError.status === 404 || apiError.status === 405) {
            return new OracleTransportError("unsupported-endpoint", "HTTP 404/405 from the Responses API; this base URL or gateway likely does not expose /v1/responses. Set OPENAI_BASE_URL to api.openai.com/v1.", apiError);
        }
