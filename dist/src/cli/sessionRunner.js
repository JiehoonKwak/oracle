...
export async function performSessionRun({ sessionMeta, runOptions, cwd, log, write, version, muteStdout = false, }) {
    ...
    try {
        ...
        if (multiModels.length > 1) {
            ...
            if (runOptions.writeOutputPath) {
                const jsonOutput = buildMultiModelJson(runOptions.prompt ?? "", summary.fulfilled.map((entry) => ({
                    model: entry.model,
