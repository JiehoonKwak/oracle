...
import { collectPaths, collectModelList, parseFloatOption, parseIntOption, parseSearchOption, usesDefaultStatusFilters, resolvePreviewMode, normalizeModelOption, normalizeBaseUrl, resolveApiModel, parseHeartbeatOption, parseTimeoutOption, parseDurationOption, mergePathLikeOptions, dedupePathInputs, } from "../src/cli/options.js";
...
program
    ...
    .option("-m, --model <model>", "Model to target (default from config or google/gemini-3.1-pro-preview). Any OpenRouter model ID works (e.g. x-ai/grok-4.1-fast).", normalizeModelOption)
    .addOption(new Option("--models <models>", 'Comma-separated model list to query in parallel (e.g., "google/gemini-3.1-pro-preview,x-ai/grok-4.1-fast").')
    ...
    .default(false)
    .hideHelp())
    .option("--render-markdown", "Print the assembled markdown bundle for prompt + files and exit.", false)
...
async function runRootCommand(options) {
    ...
    await pruneOldSessions(retentionHours, jsonOutput ? () => { } : (message) => console.log(chalk.dim(message)));
    ...
    if (detached === false) {
        await runInteractiveSession(sessionMeta, liveRunOptions, false, true, process.cwd(), jsonOutput);
        return;
    }
    if (detached) {
...
async function runInteractiveSession(sessionMeta, runOptions, showReattachHint = true, suppressSummary = false, cwd = process.cwd(), jsonOutput = false) {
    ...
    try {
        ...
        if (jsonOutput && result) {
            const json = result.answers.length === 1
